...
A process consists of several components that work together. The **Code Section**, also called the Text section, contains the compiled instructions of your program. The **Data Section** holds global and static variables that persist throughout the program's lifetime. The **Heap** is where dynamically allocated memory lives — when you use `new` in C++, that memory comes from the heap. The **Stack** manages function calls, storing local variables, return addresses, and function parameters. Finally, the **Process Control Block (PCB)** is metadata that the OS maintains about the process, which we'll discuss in detail shortly.
